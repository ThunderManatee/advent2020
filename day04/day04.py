...
def byr(value): return 1920 <= int(value) <= 2002
def iyr(value): return 2010 <= int(value) <= 2020
def eyr(value): return 2020 <= int(value) <= 2030
def hcl(value): return re.search(r"#[0-9a-f]{6}\Z", value)
def ecl(value): return value in ["amb", "blu", "brn", "gry", "grn", "hzl", "oth"]
def pid(value): return re.search(r"\A\d{9}\Z", value)
def hgt(value):
    ...
    if len(f) == len(presence):
        ...
        for field in presence:
            ...
            if not isValid:
                break
        if isValid: part2 += 1
print(f"Passports w/ all fields: {part1}")
print(f"Passports w/ all fields and valid values: {part2}") 
